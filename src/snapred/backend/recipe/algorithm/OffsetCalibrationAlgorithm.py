...
class OffsetCalibrationAlgorithm(PythonAlgorithm):
    ...
    def PyExec(self):
        ...
        # get the ingredients
        ExtractionIngredients(**json.loads(self.getProperty("ExtractionIngredients").value))
        ...
        for group in groupingList:
            ...
            DiffractionFocussing(
                InputWorkspace=correctedWS, GroupingWorkspace=focusWS.name(), OutputWorkspace=f"_DSP_{run}_cal_{group}"
            )

            ConvertUnits(
                InputWorkspace=f"_DSP_{run}_cal_{group}", OutputWorkspace=f"_DSP_{run}_cal_{group}", Target="TOF"
            )
            ...
            for subGroup in GroupIDs:
                ...
                peaks, peakBoundaries = snp.removeOverlappingPeaks(calibrantCif, sPrm, gPrm, subGroup)
