...
class GroupByGroupCalibration(PythonAlgorithm):
    ...
    def PyExec(self):
        ...
        for subGroup in GroupIDs:
            ...
            fwhm = max(calibPeakListSorted)*self.gPrm["delDOverD"][int(subGroup)-1]*2.35
            ...
            peaks,peakBoundaries = snp.removeOverlappingPeaks(
                calibrantCif, # calibrantCif is a path to a CIF file
                self.sPrm,
                self.gPrm,
                subGroup
            )
            ...
            self.mantidSnapper.CloneWorkspace(
                "Copy the input",
                InputWorkspace=rawInputWS,
                OutputWorkspace= f'_TOF_{self.runNumber}_cal_CC_PD'
            )
