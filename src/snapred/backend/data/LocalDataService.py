...
class LocalDataService:
    ...
    def writeNormalizationRecord(self, record: NormalizationRecord, version: int = None):  # noqa: F821
        ...
        # There seems no need to write the _nested_ Normalization, because you're also saving it using 'writeNormalizationState'.
        # However, if you really want to _nest_ it, at least mark it with the correct version.  :(
        # (Please use pydantic Field(exclude=True) to _stop_ nesting it.)
    ...
    def writeCalibrationRecord(self, record: CalibrationRecord, version: int = None):
        ...
        # There seems no need to write the _nested_ Calibration, because you're also saving it using 'writeCalibrationState'.
        # However, if you really want to _nest_ it, at least mark it with the correct version.  :(
        # (Please use pydantic Field(exclude=True) to _stop_ nesting it.)
    ...
    def _readGroupingMap(self, stateId) -> GroupingMap:
        groupingMapPath = self._groupingMapPath(stateId)
        if groupingMapPath.exists():
            return parse_file_as(GroupingMap, groupingMapPath)
        # Note: <grouping map>.stateId is updated from its default value at <instrument state>: root_validator.  _Not_ here.
        return parse_file_as(GroupingMap, self._defaultGroupingMapPath())
    ...
    def _defaultGroupingMapPath(self) -> Path:
        return Path(Config["instrument.calibration.powder.grouping.home"]) / 'defaultGroupingMap.json'

    def _groupingMapPath(self, stateId) -> Path:
        return self.instrumentConfig.calibrationDirectory / 'Powder' / str(stateId) / 'groupingMap.json'
