...
from snapred.backend.dao.ReductionState import ReductionState  # noqa: E402
from snapred.meta.Config import Resource, Config
from mantid.simpleapi import mtd, DeleteWorkspace, Load, CompareWorkspaces, ConvertToEventWorkspace
...
def compareWorkspaces(expected, actual):
    ...
    assert CompareWorkspaces(expected, actual) == True
...
def test_happypath_calibration():
    ...
    for output_file in expected_outputs_files:
        ...
        Load(Resource.getPath(outputs_root + output_file), NumberOfBins=2000, OutputWorkspace=output)
